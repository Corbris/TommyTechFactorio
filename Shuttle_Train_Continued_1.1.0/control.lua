...
--1.71 addition
local sFullGUI = "mg-shuttle-full-GUI"
local sExitAction = "mg-shuttle-exit-action"
local sDepotName = "mg-shuttle-depot-name"
local sGuiButtons = "mg-shuttle-gui-size"
local getSetting
do
	local map = {
		[sClearFilters] = true,
...
local function findDepot(p)
		...
		if stations and #stations ~= 0 then
			for _, station in next, stations do
				if station.backer_name == getSetting(p, sDepotName) then
					return true
				end
			end
		else 
...
do
	...
	getStations = function(p, index)
		local s = global.page[index] and ((global.page[index] - 1) * getSetting(p, sGuiButtons) + 1) or 1
		if s < 1 then s = 1 end
...
do
	...
	local function doUpdate(f, p, ...)
		for i = 1, getSetting(p, sGuiButtons) do
			local s = (select(i, ...))
	...
	updateStationInterface = function(frame, p)
		...
		frame.two.clear()
		doUpdate(frame.two, p, getStations(p, p.index))
	end
...
do
	...
	local function initGlobals()
		...
		if not global.lastusedShuttle then global.lastusedShuttle = "" end
	end
	...
	table.insert(onConfChangedFuncs, function(data)
		...
		if data.mod_changes["mg-shuttle"] then
			updateIfVisible(game.player.index)
		end
	end)
...
do
	...
	local function leftVehicle(p)
			...
			if global.lastusedShuttle and global.lastusedShuttle.valid then
				lastusedShuttle=global.lastusedShuttle
			end
			if lastusedShuttle and lastusedShuttle.valid then
				if getSetting(p, sExitAction) == "Hibernate" then
					lastusedShuttle.manual_mode = true
					lastusedShuttle.schedule = nil
				elseif getSetting(p, sExitAction) == "Depot" then
					if findDepot(p) then
						p.print("Returning shuttle to "..getSetting(p, sDepotName))
						lastusedShuttle.schedule = {
						current = 1,
						records = {
						{
							station = getSetting(p, sDepotName),
							wait_conditions = waitConditions,
						}
						}
						}
						lastusedShuttle.manual_mode = false
					elseif not findDepot(p) then
						p.print("No station called " ..getSetting(p, sDepotName).. " found on this surface, build one to enable this functionality")
					end
				elseif getSetting(p, sExitAction) == "Manual" then
					lastusedShuttle.manual_mode = true
				elseif getSetting(p, sExitAction) == "None" then
				end
			end
	end
	...
	script.on_event(defines.events.on_player_driving_changed_state, function(event)
		...
		if p.vehicle and p.vehicle.valid then
			...
			if p.vehicle.unit_number and hasEquipment(p.vehicle.train.front_stock) then
				lastusedShuttle = p.vehicle.train
				global.lastusedShuttle=p.vehicle.train
			...
			if inShuttle[event.player_index] then
				leftVehicle(p)
			end
		end
	end)
			...
			if p.opened and p.opened.type and p.opened.type == "train-stop" then
				...
				if stations and #stations ~= 0 then
					for _, station in next, stations do
						...
						if global.filter[p.index] ~= nil then
							tempfilter=string.lower(global.filter[p.index])
							tempname=string.lower(station.backer_name)
							if tempname:find(tempfilter, 1, true) then
								if not lowestDistance or distance < lowestDistance then
									lowestDistance = distance
...
do
	...
	local function keyCombo2(event)
		local p = game.players[event.player_index]
		local depotStation=findDepot(p)
		if not depotStation then
			p.print("No station called Shuttle Train Depot found on this surface")
		end
		if global.lastusedShuttle and global.lastusedShuttle.valid then
				lastusedShuttle=global.lastusedShuttle
			end
		if depotStation and lastusedShuttle and lastusedShuttle.valid then
			p.print("Sending last used shuttle to ".. getSetting(p, sDepotName))
			lastusedShuttle.schedule = {
				current = 1,
				records = {
					{
	...
	script.on_event("last-shuttle-to-depot", keyCombo2)
end
