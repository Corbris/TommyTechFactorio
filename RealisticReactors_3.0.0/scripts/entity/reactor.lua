...
local function remove_reactor(entity, tick, has_died)
	...
	if reactor.state ~= 0 then
		...
		fallout.create(surface, dead_reactor_core.position, tick)
	end --meltdown
	...
	if has_died and reactor.state ~= 0 then
		...
		ruin.add(dead_reactor_name, dead_reactor_core)
	else
		reactor.interface.destroy() -- remove interface entity
	end
...
-- 		reactor.control.parameters = {}
		...
		if game.tick - reactor.interface_warning_tick >= 120 then
			reactor.interface_warning = create_warning(reactor.interface)
	...
	if fluid == nil then
		...
		if not cooled then
			...
			if game.tick - reactor.cooling_warning_tick >= 120 then
				reactor.cooling_warning = create_warning(reactor.entity)
...
local function update_reactor_states(reactor, tick)
	...
	elseif reactor_state == 1 then
		...
		elseif running_time >= Setting.reactor_duration("starting") then
			state_changed = true
...
-- 			game.players[1].print("The starting phase of a nuclear reactor was aborted, because the reactor core didn't get enough electric energy from the grid. The fuel cell is lost.")
	...
	elseif reactor_state == 2 then
		...
		if signal_scram == true then
			state_changed = true
			change_reactor_state(3, reactor, tick)
		elseif reactor.entity.get_fuel_inventory().is_empty() and reactor.entity.burner.remaining_burning_fuel == 0 and reactor.entity.burner.currently_burning == nil then
			state_changed = true
			change_reactor_state(0, reactor, tick)
		end

	elseif reactor_state == 3 then
		if running_time >= Setting.reactor_duration("scram") then
			state_changed = true
	...
	--update reactor signals
	if reactor_state  == 1 then
		reactor.signals.parameters["state_starting"].count = Setting.reactor_duration("starting") - running_time
		reactor.signals.parameters["neighbour-bonus"].count = 0
	end
	if reactor_state  == 3 then
		reactor.signals.parameters["state_scramed"].count = Setting.reactor_duration("scram") - running_time
		reactor.signals.parameters["neighbour-bonus"].count = 0
	end
	if reactor_state  == 0 then
	...
	if reactor_state == 1 or reactor_state == 2 or reactor_state == 3 then
		...
		-- if reactor.state  == 1 then
			-- reactor.signals.parameters["state_starting"].count = Setting.reactor_duration("starting") - running_time + 1 --ToDo: starting phase should end, when first fuel cell is half empty
			-- reactor_parameters.power = math.floor(reactor_parameters.power * ((running_time)/Setting.reactor_duration("starting"))^2)
			-- --reactor_parameters.efficiency = reactor_parameters.efficiency
			-- reactor_parameters.bonus_cells = 0
		-- end
		-- if reactor.state  == 3 then
			-- reactor.signals.parameters["state_scramed"].count = Setting.reactor_duration("scram") - running_time + 1
			-- reactor_parameters.power = math.floor(reactor_parameters.power * ((Setting.reactor_duration("scram") - (running_time)/3.5)/Setting.reactor_duration("scram"))^11+0.45)
			-- --reactor_parameters.efficiency = reactor_parameters.efficiency
			-- reactor_parameters.bonus_cells = 0
		-- end
