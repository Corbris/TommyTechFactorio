...
local function merge_heat_network(a,b, position)
	local an,bn = a.chunk.network, b.chunk.network
	if an == bn then return a end
	if cell_score(a) < cell_score(b) then a,b = b,a  an,bn = bn,an end -- spare some work
-- 	log(string.format("merge heat network %s into %s  %s", bn.id, an.id, serpent.line{from=bn.count,to=an.count}))
	...
	return sameposition(a.chunk.position, position) and a or b
end

local function add_to_heat_network(x,y,z, position, old_cell)
	if sameposition(position, old_cell.chunk.position) then return old_cell end
	local cell = create_heat_network_cell(x,y,z, old_cell.chunk.network)
	set_heat_network_cell(x,y,z, cell)
-- 	log(string.format("added new cell to network %s at x=%s y=%s z=%s", cell.chunk.network.id, x,y,z))
	return cell
end

local function merge_heat_network_cells(x,y,z,position, a,b)
	if not b or a == b then return a elseif not a then return add_to_heat_network(x,y,z,position,b) end
	if not sameposition(a.chunk.position, b.chunk.position) then return merge_heat_network(a,b, position) end
...
-- 	a.area = growarea(a.area, b.area)
	...
	return merge_heat_network(a,b, position)
end


local function add_entity_to_network_cell(x,y,z,entity)
	local position,cell = Vector(ChunkCoordinates(x,y,z))
...
-- 		log(string.format("looking at x=%s y=%s for heat network cell", o.x,o.y))
		cell = merge_heat_network_cells(x,y,z,position, cell, get_heat_network_cell(o.x,o.y,z))
...
-- 			log(string.format("test x=%s y=%s for outlet with %s reactors", p.x,p.y, table_size(reactors)))
			for _,reactor in pairs(reactors) do
				...
				cell = merge_heat_network_cells(x,y,z,position, cell, get_heat_network_cell(q.x,q.y,z))
			end
		end
	end
	cell = cell or create_heat_network(x,y,z)
	set_heat_network_cell(x,y,z, cell)
...
-- 	log(string.format("add reactor to network  x=%s y=%s z=%s", x,y,z))
	...
	reactor.cell = cell
	-- merge outlets
	-- FIXME outlet position needs to be continuesly neighboured to reactor position for borders
	local add,corner = {},{cell} -- up to 4 cells
...
-- 		log(string.format("outlet at x=%s y=%s z=%s with %s others", o.x,o.y,z, table_size(reactors)))
		...
		other = merge_heat_network_cells(p.x,p.y,z,p, other, get_heat_network_cell(o.x,o.y,z))
		corner[i] = other
		set2d(add, p.x,p.y, other)
	end
	for x,xs in pairs(add) do
		for y,other in pairs(xs) do
			other.count.outlet = other.count.outlet + 1
			set_heat_network_cell(x,y,z, other)
			set_chunk_borders(other.chunk, x,y, other)
			set2d(other.entities, x,y, entity)
		end
	end
	debug.update()
end
