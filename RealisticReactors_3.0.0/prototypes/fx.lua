...
local perma_radiation = {
	...
	cyclic = true,
	duration = Setting.duration("fallout") * 60,
	fade_away_duration = math.min(180,Setting.duration("fallout")/3) * 60,
	--fade_in_duration = math.min(5,Setting.duration("fallout")/3) * 60, --doesn't work
	spread_duration = math.min(5,Setting.duration("fallout")/3) * 60,
...
data:extend{
	...
	-- fallout cloud
	{
		...
		cyclic = true,
		duration = Setting.duration("clouds") * 60,
		--fade_in_duration =  math.min(20,Setting.duration("clouds")/3) * 60, --doesnt work
		fade_away_duration = math.min(20,Setting.duration("clouds")/3) * 60,
		--spread_duration = 50,
		spread_duration = 300,
